...
for (j in 1:num_sims) {
  ...
  for (i in 1:1) {
    ...
    for(u in 1:10){
      ...
      # Checking error
      if((1 - confusion_nn_train$overall[1]) < min_error){
        ...
        # Updating error
        min_error = 1 - confusion_nn_train$overall[1]
...
plot1 <- ggplot(stack(df_MSPE), aes(x = ind, y = values)) +
...
final_sim1_results = data.frame(Error = colMeans(final_results, na.rm = T),
...
# Saving table
write.table(final_sim1_results, file="AverageSim1Pred_class.csv", row.names = F)
